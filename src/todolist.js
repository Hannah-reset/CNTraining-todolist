...
function addTodoItem(e) {
  ...
  li.setAttribute(
    'style',
    'font-size: 2rem; padding-top: 0.6rem; word-break: break-all;'
  )
...
function checkboxOnclick(checkbox) {
  ...
  if (!document.getElementById('radio-all').checked) {
    document
      ...
      .removeChild(checkbox.parentElement)
  }
...
function radioChange(radio) {
  ...
  allItems.forEach((item) => {
    ...
    if (item.firstChild.checked && radio.value === 'Active') {
      liItems.removeChild(item)
    }
    if (!item.firstChild.checked && radio.value === 'Finished') {
      liItems.removeChild(item)
    }
    if (item.firstChild.checked && radio.value === 'Clear completed') {
      liItems.removeChild(item)
